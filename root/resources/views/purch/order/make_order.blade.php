...
<script type="text/javascript">	
	...
	function onChangeUnit(field, row, pr_unit, pr_qty){
		...
		$(".line_qty_"+row).val(new_qty.toFixed(4));
		$(".line_qty_orig_"+row).val(new_qty.toFixed(4));
	...
	function initTable(data){
		...
		var disc_perc = parseFloat(data.disc_perc);
		var disc_usd = parseFloat(data.disc_usd);
		$("#table-income tbody").append('<tr>'+
				...
				'</td>'+
				'<td>'+
				'	<input type="number" value="'+qty.toFixed(4)+'" length="50" step="any" class="form-control noscroll line_qty line_qty_'+data.id+'" onChange="enterQty(this, '+data.id+')"  name="line_qty[]" placeholder="{{trans('lang.enter_text')}}"/>'+
				'	<input type="hidden" value="'+qty.toFixed(4)+'" class="line_qty_orig line_qty_orig_'+data.id+'"/>'+
				'</td>'+
				'<td>'+
				'	<input type="number" value="'+price.toFixed(3)+'" length="50" step="any" class="form-control noscroll line_price line_price_'+data.id+'" onkeyup="calculateRecord('+data.id+')"  name="line_price[]"/>'+
				'</td>'+
				'<td>'+
				'	<input type="text" readonly value="'+amount.toFixed(2)+'" class="form-control noscroll line_amount line_amount_'+data.id+'" name="line_amount[]"/>'+
				'</td>'+
				'<td>'+
				'	<input type="number" length="50" step="any" class="form-control noscroll line_disc_perc line_disc_perc_'+data.id+'" onkeyup="onDiscountPercentage(this, '+data.id+')" value="'+disc_perc.toFixed(3)+'" name="line_disc_perc[]"/>'+
				'</td>'+
				'<td>'+
				'	<input type="number" length="50" step="any" class="form-control noscroll line_disc_usd line_disc_usd_'+data.id+'" onkeyup="onDiscountDollar(this, '+data.id+')" value="'+disc_usd.toFixed(3)+'" name="line_disc_usd[]"/>'+
				'</td>'+
				'<td>'+
				'	<input type="text" readonly value="'+total.toFixed(2)+'" class="form-control noscroll line_grend_total line_grand_total_'+data.id+'" name="line_grend_total[]"/>'+
				'</td>'+
				'<td>'+
	...
	$(document).ready(function(){
		...
		if(jsonRecord = GetDetail('{{$obj->id}}')){
			var last_total = parseFloat('{{floatval($obj->grand_total) - floatval($obj->deposit)}}');
			var grand_total = parseFloat('{{$obj->grand_total}}');
			...
			$(".last_total").val(last_total.toFixed(2));
